...
class restore_qtype_preg_plugin extends restore_qtype_plugin {
    ...
    /**
     * Process qtype element using extra_question_fields.
     */
    public function process_extra_question_fields($data) {
        global $DB;
        $pluginname = $this->pluginname;
        $qtype = question_bank::get_qtype($pluginname);
        $extraquestionfields = $qtype->extra_question_fields();
        ...
        if ($questioncreated) {
            // Adjust some columns.
            $data->$questionfield = $newquestionid;
            $this->remap_extra_question_fields($data);
            // Insert record.
            $newitemid = $DB->insert_record($extraquestiontable, $data);
            // Create mapping.
            $this->set_mapping($extraquestiontable, $oldid, $newitemid);
        } else {
